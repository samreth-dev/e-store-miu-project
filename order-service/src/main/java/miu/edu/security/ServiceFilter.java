...
public class ServiceFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (request.getRequestURI().contains("/api/orders/update-status/")) {
            ...
            if (serviceToken != null) {
                validated = jwtHelper.validateServiceToken("payment-service", serviceToken)
                        ||jwtHelper.validateServiceToken("bank-service", serviceToken)
                        ||jwtHelper.validateServiceToken("credit-service", serviceToken)
                        ||jwtHelper.validateServiceToken("paypal-service", serviceToken)
                        ||jwtHelper.validateServiceToken("shipment-service", serviceToken);
            }
        }
        if (validated)
